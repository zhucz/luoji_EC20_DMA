...
void Show_Font(u16 x,u16 y,u8 *font,u8 size,u8 mode)
{
	...
	{ //15标示自建汉字库中的个数，循环查询内码
		if ((codeHZ_32[k].Index[0]==font[0])&&(codeHZ_32[k].Index[1]==font[1]))
		{ 
			break;
	...
	for(t=0;t<csize;t++)
	{   												   
		temp=codeHZ_32[k].Msk[t];			//得到点阵数据                          
		for(t1=0;t1<8;t1++)
		{
			if(temp&0x80)LCD_Fast_DrawPoint(y,x,POINT_COLOR);
			else if(mode==0)LCD_Fast_DrawPoint(y,x,BACK_COLOR); 
			temp<<=1;
			y++;
			if((y-y0)==size)
			{
